...
func (m *MovieScope) Search(query *scopes.CannedQuery, metadata *scopes.SearchMetadata, reply *scopes.SearchReply, cancelled <-chan bool) error {
	...
	if queryStr == "" {
		...
		switch dptId {
		case "":
			m.searchMovie(metadata, reply, queryStr)
			m.searchCinema(metadata, reply, queryStr)
		case "hot_movie":
			m.searchMovie(metadata, reply, queryStr)
		case "nearby_cinema":
			m.searchCinema(metadata, reply, queryStr)
...
func (m *MovieScope) Preview(result *scopes.Result, metadata *scopes.ActionMetadata, reply *scopes.PreviewReply, cancelled <-chan bool) error {
	...
	} else if resType == "movie_search" {
		// 搜索播放影片的影院的结果
		m.viewSearchMovie(result, reply)
	} else if resType == "cinema_search" {
		// 搜索播放影片的影院的结果
		m.viewSearchCinema(result, reply)
	}
	return nil
}
...
func (m *MovieScope) showHome(metadata *scopes.SearchMetadata, reply *scopes.SearchReply) {
	...
	jsonData, err := GetAPIData(API_HOT_MOVIE, args)
	var movies []interface{}
	if err == nil {
		movies = jsonData.GetPath("result", "movie").MustArray()
	}
	...
	for _, movie := range movies {
		...
		attrs = append(attrs, map[string]interface{}{
			"icon":  "",
			"value": "★ " + fmt.Sprint(movie["movie_score"]),
		})
...
func (m *MovieScope) showHotMovies(metadata *scopes.SearchMetadata, reply *scopes.SearchReply) {
	...
	for _, movie := range movies {
		...
		attrs = append(attrs, map[string]interface{}{
			"icon":  "",
			"value": fmt.Sprint(movie["movie_tags"]),
		})
		result.Set("attributes", attrs)
...
func (m *MovieScope) showNearbyCinemas(metadata *scopes.SearchMetadata, reply *scopes.SearchReply) {
	...
	jsonData, err := GetAPIData(API_NEARBY_CINEMA, args)
	...
	for _, cinema := range cinemas {
		...
		result.Set("subtitle", "距离 "+fmt.Sprint(cinema["distance"]))

		attrs := []map[string]interface{}{}
		attrs = append(attrs, map[string]interface{}{
			"icon":  "",
			"value": "★ " + fmt.Sprint(cinema["rating"]),
		})
		result.Set("attributes", attrs)
...
func (m *MovieScope) viewHotMovie(result *scopes.Result, reply *scopes.PreviewReply) {
	...
	text.AddAttributeValue("title", "<b>信息</b>")
	...
	description.AddAttributeValue("title", "<b>描述</b>")
	...
	nearbyCinemas.AddAttributeValue("title", "<b>附近放映的影院</b>")
...
// 搜索播放影片的影院
func (m *MovieScope) searchMovie(metadata *scopes.SearchMetadata, reply *scopes.SearchReply, queryStr string) {
	...
	for _, cinema := range cinemas {
		...
		result.Set("type", "movie_search")
		result.Set("loc", loc)
		result.Set("map", cinema)
		result.SetTitle(fmt.Sprint(cinema["name"]))
		result.SetArt(m.base.ScopeDirectory() + "/local.png")
		result.SetURI(m.base.ScopeDirectory() + "/local.png")
		result.Set("subtitle", cinema["address"])
...
// 搜索播放影片的影院
func (m *MovieScope) searchCinema(metadata *scopes.SearchMetadata, reply *scopes.SearchReply, queryStr string) {
	...
	jsonData, err := GetAPIData(API_SEARCH_CINEMA, param)
	...
	category := reply.RegisterCategory("search_cinema", "附近影院搜索", "", searchMovieTemplate)
...
func (m *MovieScope) viewSearchMovie(result *scopes.Result, reply *scopes.PreviewReply) {
	...
	lng := float64(0)
	lat := float64(0)
	location, ok := cinema["location"].(map[string]interface{})
	if ok {
		lng = location["lng"].(float64)
		lat = location["lat"].(float64)
	}
	...
	info.AddAttributeValue(
		"text",
		"<b>电话: </b>"+fmt.Sprint(cinema["telephone"])+"<br/>"+
			"<b>评分: </b>"+fmt.Sprint(cinema["rating"]))
	...
	// 场次信息
	timeTable := scopes.NewPreviewWidget("timeTable", "expandable")
	timeTable.AddAttributeValue("collapsed-widgets", 2)
	timeTable.AddAttributeValue("title", "<b>场次信息</b>")

	tableData, ok := cinema["time_table"].([]interface{})
	if ok {
		for i, t := range tableData {
			t := t.(map[string]interface{})
			table := scopes.NewPreviewWidget("stable"+strconv.Itoa(i), "text")
			table.AddAttributeValue("title", "")
			...
			text := "<b>时间:</b> " + date + " " + time +
				"<br/><b>类型:</b> " + lan + _type +
				"<br/><b>价格:</b> " + price
			table.AddAttributeValue("text", text)

			timeTable.AddWidget(table)
		}
	}
...
func (m *MovieScope) viewSearchCinema(result *scopes.Result, reply *scopes.PreviewReply) {
